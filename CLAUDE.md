...
- **[a-dev-v1.4.2](https://github.com/Venser724/GC_Artemis-Firmware/releases/tag/a-dev-v1.4.2)** — fix a memory leak in the event-dispatch code (commit `c187ffa`). `Events::post` allocates a small copy of each event per subscriber and hands it to that subscriber's queue, but ignored the non-blocking send's return value — if a subscriber's queue was already full, the copy was silently dropped and never freed. Found while investigating a coredump: `BTU_TASK` aborted on a failed 84-byte internal-RAM allocation during a BLE pairing attempt, and `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=256` forces allocations this small into that same scarce pool — `MainMenu`'s `Facility::Phone` queue (depth 4) sits undrained whenever `MainMenu` isn't the active screen, so a burst of phone-connection events (e.g. repeated BLE pairing attempts) would leak steadily. Not yet confirmed as the root cause of that crash. Attached asset `Artemis-a-dev-v1.4.2.bin` is a complete 4MB flash image, same flashing method as v1.0.
- **[a-dev-v1.4.1](https://github.com/Venser724/GC_Artemis-Firmware/releases/tag/a-dev-v1.4.1)** — fix a rare crash after long uptime (commit `fed7d58`). A coredump pulled from a watch running ~1 week showed the UI task aborting on a failed small internal-RAM allocation, same shape as two earlier fragmentation fixes but via a different path this time: the weather condition icon and the phone-connection unread-notification badge were the only frequently-redrawn images not preloaded into `FSLVGL`'s image cache, so every redraw fell through to a slow `fopen()`/`fclose()` instead of the cached path, fragmenting memory over long uptime. Both are now preloaded like everything else. Attached asset `Artemis-a-dev-v1.4.1.bin` is a complete 4MB flash image, same flashing method as v1.0.
- **[a-dev-v1.4](https://github.com/Venser724/GC_Artemis-Firmware/releases/tag/a-dev-v1.4)** — icon-based media controls and reliable notifications navigation (commit `31fcba3`). `MusicScreen`'s prev/play-pause/next hints are now icons (`mediaIcons.c`, a custom 1bpp font generated from LVGL's own bundled FontAwesome subset to match the project's crisp pixel-font style) instead of text, with the play/pause icon reflecting current state; empty now-playing state shows "No media"/"No artist" instead of a blank "—". On the lock screen, pressing Down now always transitions into the notifications panel (added a permanent "No notifications" placeholder kept in sync with the notification count, since the panel was previously unreachable via Down with zero notifications), and that transition (and the media-screen one) is instant with no border, matching each other. Attached asset `Artemis-a-dev-v1.4.bin` is a complete 4MB flash image, same flashing method as v1.0.
- **[a-dev-v1.1](https://github.com/Venser724/GC_Artemis-Firmware/releases/tag/a-dev-v1.1)** — live weather widget on the lock screen (commit `4f552ee`): a `Weather` service fed by GadgetBridge's Bangle.js `weather` message (temp/hi/lo/rain/condition), sourced from any GadgetBridge-compatible weather provider app on the phone (tested against Breezy Weather — GadgetBridge itself has no internet permission and fetches nothing on its own). Also adds Cyrillic notification text support and day-of-week on the lock screen date. Attached asset `Artemis-a-dev-v1.1.bin` is a complete 4MB flash image, same flashing method as v1.0.
